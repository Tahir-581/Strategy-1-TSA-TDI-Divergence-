//@version=4
//addition of Bullish candle confirmation for buys and Bearsish candle confirmation for sells
strategy("6--TSA TDI Divergence Strategy", overlay = true, max_bars_back = 5000, default_qty_type = strategy.percent_of_equity, default_qty_value = 100)
...
// Remove old lines and labels if show only last divergence option is enabled
...
// For buy signals: First leg of divergence must be below lower band AND oversold level
buy_filter = pos_reg_div_detected and tdi_div_below_lower

// For sell signals: First leg of divergence must be above upper band AND overbought level
sell_filter = neg_reg_div_detected and tdi_div_above_upper
...
//alertcondition(sell_condition, title='TSA TDI Sell Signal', message='TSA TDI Sell
